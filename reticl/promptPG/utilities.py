"""
This file is copied from https://github.com/lupantech/PromptPG/blob/main/run_gpt3_rl/utilities.py
Attribution goes to the original authors of https://arxiv.org/pdf/2209.14610.pdf (Lu et al., 2022)

A small modification was made:
Originally, if a predicted MC option fell outside the pre-defined range, a random option would be selected.
Additionally, if no regex pattern was matched for a MC question, the most string similarity would select the closest option.
Now, an option is provided to turn off this functionality and return an empty string instead of using this optimistic recovery.
"""
...
def extract_prediction(output, options, option_inds, forgiving):
    # $\\frac{16}{95}$ -> 16/95
